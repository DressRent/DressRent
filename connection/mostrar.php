...
$query = "SELECT id_prenda, descripcion, tipo, precio, existencias, imagen FROM prendas";
...
if ($result) {
    while ($row = pg_fetch_assoc($result)) {
        ...
        if (!empty($row['imagen'])) {
            echo '<img class="card-img rounded-0 img-fluid" src="data:image/png;base64,' . base64_encode($row['imagen']) . '">';
        } else {
